...
\subsection{Overview: one training step, end to end}
GoldiMask is one method with four components, each specified in its own subsection below and assembled as follows (Figure~\ref{fig:overview}). A step draws its noise level and over-masks as above. \emph{(i)} A gradient-free first pass reads each masked candidate's confidence $p_i$ and the attention map $A$. \emph{(ii)} Up to a budget set by the thermostat (component iv), a greedy maximization of the objective $F$ (component ii, built from the two measured curves of component i) chooses which candidates to reveal back as context. \emph{(iii)} The training pass supervises the remaining masked tokens, each weighted by its \emph{realized} gain from the revealed context, allocated by an exact fair-division rule (component iii). \emph{(iv)} The mean post-reveal confidence of the supervised tokens is compared to the measured learnability peak $p^\star$, and a two-line dual-ascent update adjusts the reveal cap for the next step---the budget is the Lagrange multiplier of the constraint $\mathbb{E}[\bar p] = p^\star$. Fixed quantities are measured curves; the one adaptive quantity is a multiplier enforcing a constraint on a measured signal. Alternative budget rules, weighting rules, and corruption designs are evaluated as ablations (Sections~\ref{sec:v2results}, \ref{sec:ablations}, and \ref{sec:refute}), not as parts of the method.
