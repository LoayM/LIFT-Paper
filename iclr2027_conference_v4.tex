...
\item \textbf{Controls, refutations, and service.} A random-reveal control separates structure from selection; wrong-shaped-$\lambda$ controls lose the gain in proportion to peak displacement; a four-variant refutation campaign (per-example budgets, full-corruption selection, their combination, step-level scaling) fails to beat the flagship anywhere, with one architecture-dependent exception reported as a finding (Section~\ref{sec:ablations}); the released LIFT code is shown unable to train Dream-7B (two bugs, fixed, raising their own result); and a paired 1-GPU/4-GPU study certifies hardware-layout equivalence for every comparison we make (Appendix~\ref{app:layout}).
\end{itemize}
...
Fine-tuning a masked diffusion model turns each training example into a lesson: some response tokens are visible and serve as material, the rest are masked and supervised. The gradient of a masked token teaches one thing---produce this token from this context---so what the model learns from a step is decided by the split itself. A random split squanders much of the lesson: a masked token whose determining evidence is also masked is unanswerable, and supervising it rewards reproducing the answer without justification; a token that is obvious from the visible remainder contributes no gradient at all. The tokens worth supervising lie between these failures---answerable from the visible evidence through an inference the model has not yet mastered. Our method is, at its core, a function that constructs this regime deliberately: at every step, given the randomly masked example, it selects which masked tokens to reveal back as context so that training on the remaining ones benefits the model most (this section). Two further components complete the method---the weighting of the supervised tokens (Section~\ref{sec:weighting}) and the size of the reveal budget (Section~\ref{sec:lagrangian})---and one principle runs through all three: every fixed quantity is a measured curve, and every adaptive quantity enforces a constraint on a measured signal.

\subsection{The selection function}
\label{sec:objective}

Which tokens to reveal is decided by maximizing a single objective. The gradient-free first pass supplies exactly two signals about each masked token $i$: the model's \emph{confidence} $p_i$ in its correct value, and its \emph{attention}---how much $i$ draws on each other position (head-averaged, recomputed from queries and keys, diagonal zeroed), giving the support $S_i(R) = \sum_{j\in R} A_{ij}$ it would receive from a candidate reveal set $R$. Our objective combines the two:
\begin{equation}
F(R) \;=\; \sum_{i \notin R} \lambda(p_i)\,\phi\big(S_i(R)\big),
\label{eq:objective}
\end{equation}
the predicted benefit of revealing $R$ (whose size is set by the budget of Section~\ref{sec:lagrangian}): each still-supervised token contributes its \emph{teaching value} $\lambda(p_i)$---how much supervising it pays off at its current confidence---scaled by $\phi(S_i(R))$, the fraction of that value the revealed context actually delivers. The factorization reflects a simple principle: a token benefits from supervision precisely when it responds to evidence while headroom remains---responsiveness being the observable signature of an inference the model has begun but not completed, the regime in which a gradient strengthens reasoning rather than memorization. Both shaping functions are instantiated from pre-training measurements (below; protocols in Appendix~\ref{sec:measurements}), and the ablations of Section~\ref{sec:ablations} probe the principle directly: displacing the measured curves degrades performance in proportion to the displacement. Two structural properties complete the machinery. The sum runs over $i \notin R$, so every reveal forfeits its own term: a candidate hint must earn more through the tokens it supports than it costs by leaving the lesson, which drives the optimizer to build context from tokens that are cheap to give away---already mastered, or currently hopeless---and to protect the teachable middle. And $F$ is submodular for any concave, nondecreasing $\phi$ with $\phi(0)=0$---conditions our $\phi$ satisfies exactly (Proposition~\ref{prop:submodular}), so greedy construction of $R$ is provably near-optimal \citep{buchbinder2014submodular} at $\sim$140ms per step; prior attention heuristics emerge as its limiting cases (Proposition~\ref{prop:limits}).

\paragraph{Teaching value $\lambda$.} A token rewards teaching when two things hold at once: the model can use evidence about it, and there is still something left to learn. The second factor is definitional---the remaining headroom $1-p$. The first is observable: if the model can use evidence about a token, revealing additional context must raise its confidence in the correct answer, and this response is directly measurable by masking the same example twice at nested levels and querying the model about the tokens hidden in both (Appendix~\ref{sec:measurements}). Combining the measured response with the headroom yields a single empirical law: teaching value depends on current confidence as an inverted U---near-hopeless tokens cannot use evidence, near-mastered tokens have nothing left to gain, and the payoff peaks at $p^\star\!\approx\!0.25$, a location that is stable across noise levels, data splits, and even architectures (Appendix~\ref{sec:measurements}). We adopt the matching closed form $\lambda(p) \propto p(1-p)^3$.

\paragraph{Support $\phi$.} Teaching value identifies whom to help; the selection must still determine which reveals help them. The model itself keeps the relevant record: attention---when predicting token $i$, how much it draws on position $j$ (head-averaged, recomputed from queries and keys, diagonal zeroed; $S_i(R) = \sum_{j\in R} A_{ij}$). We validate and calibrate this record by fixing the supervised targets and revealing randomized sets (Appendix~\ref{sec:measurements}): a masked token's confidence gain is predicted by the attention mass it places on the revealed set, monotonically and with diminishing returns---a token needs \emph{enough} relevant context, not all of it. The form and the constant are both read off the data: the one-parameter saturating family $s/(s+s_0)$ fits the measured (support, benefit) pairs better than linear and hard-saturating alternatives, the half-saturation $s_0$ fitted per noise bin spans $0.05$--$0.2$, and we adopt the mid-range default $\phi(s) = s/(s+0.1)$---a choice the selection is insensitive to, as $2\times$ perturbations of $s_0$ leave token rankings essentially unchanged (rank correlation $\geq 0.92$). 

\subsection{Loss weighting: realized gain, fairly allocated}
\label{sec:weighting}

Selection chose the reveals using \emph{predicted} benefit; by the end of the supervised pass, that prediction has been checked by reality, and the loss should spend the information. For each supervised token, the two passes provide its confidence before the reveals ($p_1$) and after ($p_2$), and we grade by demonstrated progress:
\begin{equation}
u_i \;=\; \max(\log p_2^{(i)} - \log p_1^{(i)},\, 0) \cdot \lambda(p_2^{(i)}).
\end{equation}
The first factor credits tokens the revealed evidence demonstrably engaged. The second---the same measured curve, now evaluated at the \emph{post-reveal} confidence---declines to pay for exercises the reveals already solved: a token pushed to near-certainty has nothing left to teach, however large its jump. Weight therefore flows to tokens the evidence moved \emph{into} the teachable zone and no further.

Utilities must then become weights, and three requirements pin the conversion down. The weights must sum to exactly $n$, so that reweighting redistributes the step's gradient without changing its size---and hence leaves the effective learning rate untouched. No token may exceed a cap $c$ ($c{=}10$ throughout): template and formatting tokens occasionally exhibit enormous nominal gains and must not absorb the step. And subject to both, weight should follow measured merit. \emph{Water-filling} is not one option among many here; it is the unique rule satisfying all three requirements simultaneously:
\begin{equation}
w_i \;=\; \min\!\big(u_i/\nu,\; c\big), \qquad \nu \text{ such that } \textstyle\sum_i w_i = n,
\end{equation}
with the level $\nu$ found by a one-dimensional bisection. (The obvious alternative---normalize, then clip, as in early versions of our own method---violates the first requirement whenever the clip binds; the ablation in Table~\ref{tab:ablations} prices that failure at $1.6$ core points under scarce reveals.)

Two results make the construction precise; formal statements and proofs are in Appendix~\ref{app:theory}.

\emph{Proposition~\ref{prop:waterfill} (fair allocation, informal).} Among all weight vectors with total $n$ and per-token cap $c$, the water-filling weights are the unique maximizer of the proportional-fairness objective $\sum_i u_i \log w_i$---equivalently, the Nash bargaining solution in which tokens bargain for the step's gradient with power equal to their measured utility.

\emph{Corollary~\ref{cor:ess} (participation floor, informal).} Under any weights respecting the cap, the effective number of tokens receiving gradient never falls below $n/c$.

The theory earns its place operationally. Budget exactness keeps every comparison in Section~\ref{sec:experiments} at a fixed effective learning rate; uniqueness means the weighting carries no tuning surface---it is the answer to a stated allocation problem, not a smoothing heuristic with settings; and the participation floor guarantees the gradient can never silently concentrate on a handful of freak tokens. One question now remains---the one every corruption-design method before ours has answered by fiat: how much help should a lesson contain?

\subsection{The budget: a constraint on measured difficulty}
\label{sec:lagrangian}

One decision remains: how much to reveal. Every prior answer to this question is a constant---a fixed fraction, a fixed count, a fixed threshold---and any such constant silently inherits the model, dataset, and training stage it was tuned on. We take a different route, available only because the target state is something we measured: we know where supervised difficulty pays off most (mean confidence at the learnability peak $p^\star$), so rather than fixing the \emph{amount} of help, we fix its \emph{consequence}, and let the amount be whatever enforces it.

Formally, we pose the constrained problem $\min_\theta \mathbb{E}[\mathcal{L}(\theta; c)]$ subject to $\mathbb{E}[\bar p(c)] = p^\star$, where $c$ is the reveal cap and $\bar p$ the supervised tokens' mean post-reveal confidence---read off the training pass at no cost. The cap becomes the Lagrange multiplier's instrument, and stochastic dual ascent on the multiplier is a two-line thermostat appended to each step:
\begin{equation}
\mu_{k+1} \;=\; \mu_k + \eta\,(\bar p_k - p^\star), \qquad
c_{k+1} \;=\; 0.1 + 0.5\,\sigma(-\mu_{k+1}),
\end{equation}
with gain $\eta = 0.02$. Steps whose exercises came out too easy raise $\mu$ and tighten the cap; too hard, and it loosens. Nothing else in training changes.

\emph{Proposition~\ref{prop:dual} (unique equilibrium, informal).} Under the single assumption that revealing more context never makes the remaining supervision harder in expectation---mechanically true, since revealed tokens only add conditioning information---the coupled iteration has exactly one fixed point: the cap at which mean supervised difficulty equals the measured peak. With fixed gain and bounded observation noise, the iterates converge to an $O(\eta)$ neighborhood of it; with decaying gain, almost surely. Formal statement and proof are in Appendix~\ref{app:theory}.

The construction's value is what it does \emph{not} contain: a transplanted constant. The target is the measured peak; the cap is discovered, by feedback, on the very model being trained---and this is precisely the property that makes the mechanism portable across architectures and datasets, where amount-based rules fail (Section~\ref{sec:v3results}). The target's location is causally validated: holding difficulty at the measured peak beats holding it above in both mean and seed variance (Section~\ref{sec:ablations}). The gain is an operating condition rather than a tuning: the fixed-gain analysis behind Proposition~\ref{prop:dual} expects overshoot at aggressive gains, and the ablations confirm it ($\eta{=}0.05$ costs two core points; Section~\ref{sec:ablations}). The same template with a different constraint---bounding the vanilla loss's degradation, with scaffold-free steps as the instrument---yields a fading controller, evaluated among the extended ablations (Appendix~\ref{app:ablations}).
...
Our ablations are of two kinds: how much each \emph{component} contributes, and whether each \emph{design choice within a component} is the right one. We give both in compressed form here; full tables, per-seed grids, and secondary screens are in Appendix~\ref{app:ablations}.

\paragraph{What each component contributes.} Table~\ref{tab:ablations} builds the method one rung at a time. The two-pass reveal structure alone---random reveal sets, no weighting---is worth $+1.9$ core points over vanilla SFT; choosing the reveals by the measured objective adds $+2.3$ more at the same budget; removing the realized-gain weighting costs $2.1$ points and, tellingly, restores a LIFT-like AIME tail (unweighted training leaves gradient on hard tokens---the memorization regime); replacing water-filling with normalize-and-clip costs $1.6$ points precisely when reveals are scarce; and the budget controller's contribution is not mean but \emph{stability and portability}---the smallest seed variance in the study ($\pm 0.2$) and, uniquely, top-band results on every architecture and corpus (Section~\ref{sec:generality}).

\begin{table}[t]
\caption{\textbf{Component ladder} (LLaDA-8B, s1K; core selected~/~final; mean $\pm$ sd over seeds 42--44 unless $^\ddagger$single-seed).}
\label{tab:ablations}
\begin{center}
\resizebox{0.86\linewidth}{!}{%
\begin{tabular}{lccl}
\toprule
Configuration & sel. & final & isolates \\
\midrule
vanilla SFT$^\ddagger$ & 36.8 & --- & the floor \\
+ reveal structure (random $R$)$^\ddagger$ & 38.7 & 38.4 & two-pass structure: $+1.9$ \\
+ measured selection \& weighting & 41.0\,$\pm$\,0.8 & 39.8\,$\pm$\,0.9 & selection at same budget: $+2.3$ \\
\quad$-$ realized-gain weighting & 38.9\,$\pm$\,0.5 & 38.1\,$\pm$\,0.3 & weighting: $+2.1$ (and the tail returns) \\
\quad$-$ water-filling (normalize-clip, scarce reveals) & 39.6\,$\pm$\,0.5 & 40.1\,$\pm$\,0.8 & fair allocation: $+1.6$ \\
\textbf{+ constraint budget (flagship)} & 41.0\,$\pm$\,0.2 & 40.8\,$\pm$\,0.5 & stability ($\pm0.2$) + portability \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}

\paragraph{Design choices: selection.} The curve \emph{content} is causal, not decorative: training with $\lambda$'s peak displaced to $p{=}0.5$ loses $1.3$ core points and displaced toward $p{=}0$ loses $3.2$---monotone in the displacement, the worst statistically at the random-reveal floor, and the pessimism control even reproduces the core-vs-tail trade-off from the curve shape alone (both single-seed; full text in Appendix~\ref{app:ablations}). $\phi$'s form is similarly pinned: its linear and hard-saturating limits correspond to the attention-hub and coverage heuristics, and both lose to the measured middle in training as in fit. Constructing the \emph{entire} visible set (full corruption) instead of amending the random draw is architecture-dependent---worse on LLaDA-8B ($40.1 \pm 0.7$), top-band on Dream ($44.9 \pm 0.5$)---and is therefore not adopted.

\paragraph{Design choices: weighting.} Across the weighting family (normalize-clip, DRO-tilted hardness, a dual-controlled hedge, step-value scaling), water-filling is the only scheme that is never worse and the best under scarce reveals; the full $3\times3$ budget$\times$weighting grid, including its reveal--weighting interaction, is Table~\ref{tab:v2grid} in Appendix~\ref{app:ablations}.

\paragraph{Design choices: budget.} The alternatives bracket the flagship from both sides. A fixed cap ($0.35$) is competitive in-domain but carries no principle and higher variance ($40.5 \pm 1.0$); marginal-priced stopping ($\varepsilon$-stop) is the strongest in-domain configuration ($41.2 \pm 0.7$, revealing only $\sim$7\% of candidates) but fails to cross architectures at any threshold (Section~\ref{sec:v3results}); the constraint's target matters ($0.25$ beats $0.35$ in mean and variance); its gain is an operating condition ($\eta{=}0.05$: $-1.9$); and making the constraint \emph{pointwise}---one thermostat per example rather than per batch---over-controls, collapsing Sudoku wherever it loses ($39.0 \pm 0.7$ on LLaDA-8B; per-setting table in Appendix~\ref{app:ablations}). In total, four independently-motivated refinements of the flagship (per-sample control, full corruption, their combination, step scaling; $41$ runs at three seeds) failed to dethrone it---the design sits at a measured optimum from both directions.

\subsection{Generality across base models and datasets}
\label{sec:generality}

We transplant the defaults configuration unchanged---same curves, same hyperparameters, no per-model tuning---to the two extension models the LIFT paper uses, at three seeds. On \textbf{Dream-7B}, GoldiMask reaches a core average of $45.1 \pm 1.6$ (selected checkpoint) and $45.5 \pm 1.2$ (final), against 43.7 for the paper's best Dream configuration (their $H{=}2$, single seed) and 36.5 for the Dream base---$+1.4$ to $+1.8$ over the strongest published result on that model. Seed 42 alone read $+3.1$; the deflation is nearly all Countdown, which carries $\pm 7$ points of seed noise on this model ($36.8 \pm 7.0$, against their 33.6). MATH500 sets a model-best $43.3 \pm 1.6$ at final checkpoints (44.8 at seed 43). Where the original work reports no AIME numbers for Dream at all, GoldiMask posts $10.0 \pm 5.8$ / $2.2 \pm 1.9$ pass@16 at protocol temperature (final checkpoints); a single seed reached 20.0 on AIME'24, which we report as the outlier it is. On \textbf{LLaDA-1.5} the same transplant lands at $40.9 \pm 1.1$ (selected) / $40.8 \pm 0.8$ (final) over three seeds: level with the paper's $H{=}2$ (41.6) but behind their single-seed $H{=}3$ (42.6), with a weaker tail ($7.8 \pm 1.9$ AIME'24)---we report this candidly; it is the model where LIFT is strongest, and while one of our seeds (42.2) reaches the published range, the mean does not. A learning-rate probe in the direction the preference-tuned model's smaller updates suggest ($2 \times 10^{-6}$) does not close the gap (41.3 selected / 40.6 final), and our diagnostic curves measured on LLaDA-1.5 are indistinguishable from LLaDA-8B's, so we cannot attribute the gap to the confidence geometry. (Appendix~\ref{app:ablations} adds the missing control: a same-harness LIFT $H{=}3$ baseline at three seeds on this model, which lands well below its published number and below both GoldiMask generations---the published 42.6 appears to be a favorable draw of the same magnitude we document for LLaDA-8B in Appendix~\ref{app:repro}.) Porting cost is worth recording: LLaDA-1.5 ran with no changes whatsoever, and Dream required a forty-line compatibility shim (a missing generation hook expected by the LoRA wrapper, and rerouting an inlined attention call so it can be observed)---the method itself transferred as-is, including the untouched default curves.
...
This appendix carries the full ablation record summarized in Section~\ref{sec:ablations}.

\subsection*{The budget-and-weighting grid, and the fixed-cap variant}
...
\subsection*{Wrong-shape controls and calibration}
...
\subsection*{Pointwise versus population constraints}
...
\subsection*{The refutation campaign}
...
\section{The measurement protocols}
\label{sec:measurements}

This appendix details the two pre-training measurements that instantiate $\lambda$ and $\phi$; the resulting curves are shown in Figures~\ref{fig:curves} and~\ref{fig:invariance}.
...
Both statements follow by substitution into the marginal above. The measured $\phi$ (Appendix~\ref{sec:measurements}) is strictly between the two limits, and Section~\ref{sec:experiments} shows the calibrated middle outperforming both.
