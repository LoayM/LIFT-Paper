...
The result (Figure~\ref{fig:curves}, left) is a lopsided inverted U. Its peak sits at $p \approx 0.22$ in all five noise bins; only the amplitude changes with $t$. The peak also survives a change of architecture: re-measured natively on Dream-7B (through the shifted-logits shim of Appendix~\ref{app:repro}, so the measurement is not a victim of the release-bug class documented there), the curve keeps its shape with the peak at $p \approx 0.26$ (Figure~\ref{fig:invariance})---the zone is a property of the learning problem, not the model family, which is what licenses using its location as a training \emph{constraint} below. A three-parameter form $\lambda(p) \propto p^a (1-p)^b$ fits each bin with log-domain $R^2$ between $0.93$ and $0.99$. The curve is not an artifact of the fit: refitting on disjoint halves of the examples reproduces cell means at $r = 0.967$ and moves the peak by $0.01$. Two common weighting heuristics fail against it in different ways: $1-p$ keeps weight on hopeless tokens whose measured gain is small, and $p(1-p)$ puts the peak at $0.5$, twice too far right.
...
\toprule
Name & GSM8K & MATH & Countdown & Sudoku & AIME '24 & AIME '25 & Core (sel.) & Core (final) \\
\midrule
\multicolumn{9}{l}{\emph{LLaDA-8B-Instruct}}\\[1pt]
\quad LLaDA          & 79.3 & 36.8 & 21.1 & 11.2 & 3.3 & \textbf{3.3} & 37.1 & --- \\
\quad Vanilla        & 79.2 & 36.2 & 17.2 & 14.5 & 6.7 & 0.0 & 36.8 & --- \\
\quad LIFT ($H{=}3$) & 79.9 & 35.9 & \textbf{28.8} & 13.5 & \textbf{12.2} & 1.1 & 39.5\,$\pm$\,0.4 & 39.6\,$\pm$\,0.6 \\
\quad \textbf{GoldiMask} & \textbf{80.3} & \textbf{39.7} & 26.6 & \textbf{17.1} & 8.9 & 1.1 & \textbf{41.0\,$\pm$\,0.2} & \textbf{40.8\,$\pm$\,0.5} \\
\quad & {\scriptsize\color{gray}$\uparrow$1.3\%} & {\scriptsize\color{gray}$\uparrow$7.9\%} & {\scriptsize\color{gray}$\uparrow$26\%} & {\scriptsize\color{gray}$\uparrow$53\%} & {\scriptsize\color{gray}$\uparrow$170\%} & {\scriptsize\color{gray}$\downarrow$67\%} & {\scriptsize\color{gray}$\uparrow$10.5\%} & \\
\midrule
\multicolumn{9}{l}{\emph{LLaDA-1.5}}\\[1pt]
\quad LLaDA-1.5$^\circ$ & 80.9 & 37.8 & 22.6 & 12.1 & \textbf{13.3} & \textbf{3.3} & 38.4 & --- \\
\quad LIFT ($H{=}3$) & 80.4 & 37.7 & \textbf{26.9} & 16.0 & 11.1 & \textbf{3.3} & 40.3\,$\pm$\,0.8 & 39.6\,$\pm$\,1.3 \\
\quad \textbf{GoldiMask} & \textbf{81.1} & \textbf{39.6} & 25.4 & \textbf{17.1} & 10.0 & 0.0 & \textbf{40.8\,$\pm$\,1.4} & \textbf{41.4\,$\pm$\,0.3} \\
\quad & {\scriptsize\color{gray}$\uparrow$0.2\%} & {\scriptsize\color{gray}$\uparrow$4.8\%} & {\scriptsize\color{gray}$\uparrow$12\%} & {\scriptsize\color{gray}$\uparrow$41\%} & {\scriptsize\color{gray}$\downarrow$25\%} & {\scriptsize\color{gray}$\downarrow$100\%} & {\scriptsize\color{gray}$\uparrow$6.3\%} & \\
\midrule
\multicolumn{9}{l}{\emph{Dream-7B}}\\[1pt]
\quad LIFT ($H{=}3$, fixed code) & 78.4 & 42.1 & 30.0 & \textbf{23.3} & \textbf{8.9} & \textbf{12.2} & 43.4\,$\pm$\,0.7 & 43.0\,$\pm$\,0.3 \\
\quad \textbf{GoldiMask} & \textbf{79.2} & \textbf{42.4} & \textbf{37.5} & 17.6 & 5.6 & 5.6 & \textbf{44.2\,$\pm$\,0.8} & \textbf{44.6\,$\pm$\,1.3} \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
Name & GSM8K & MATH & Countdown & Sudoku & AIME '24 & AIME '25 & Core (sel.) & Core (final) \\
\midrule
\quad Instruct$^\circ$ & 78.2 & 36.8 & 20.0 & 11.8 & 3.3 & \textbf{3.3} & 36.7 & --- \\
\quad LIFT ($H{=}2$) & \textbf{81.4} & 36.6 & 23.6 & 11.2 & \textbf{10.0} & \textbf{4.4} & 38.2\,$\pm$\,0.4 & 38.6\,$\pm$\,0.3 \\
\quad \textbf{GoldiMask} & 80.5 & \textbf{37.5} & \textbf{28.1} & \textbf{16.5} & 3.3 & 1.1 & \textbf{40.7\,$\pm$\,0.7} & \textbf{39.6\,$\pm$\,0.7} \\
\quad & {\scriptsize\color{gray}$\uparrow$2.9\%} & {\scriptsize\color{gray}$\uparrow$1.9\%} & {\scriptsize\color{gray}$\uparrow$41\%} & {\scriptsize\color{gray}$\uparrow$40\%} & {\scriptsize\color{gray}$\uparrow$0\%} & {\scriptsize\color{gray}$\downarrow$67\%} & {\scriptsize\color{gray}$\uparrow$10.9\%} & \\
\bottomrule
\end{tabular}}
\end{center}
\end{table}
...
thermostat gain $\eta{=}0.05$ instead of $0.02^\ddagger$ & 39.1 & --- & the gain is an operating condition (Prop.~\ref{prop:dual}) \\
...
\subsection{GoldiMask as constrained optimization: every knob a multiplier}
\label{sec:lagrangian}

The v2 budgets invite one further step, and it turns out to unify the method's adaptive machinery. Fix a target for a \emph{measured} quantity of training, and let the corresponding knob be the Lagrange multiplier that enforces it. Two instantiations:
...
\subsection{The v2 grid, and a budgeted champion}
\label{sec:v2results}
...
\begin{table}[t]
\caption{The v2 screening grid: core average at the selected checkpoint (AIME'24/'25 pass@16 in parentheses), LLaDA-8B, seed 42. References: GoldiMask defaults 41.8 (6.7/0.0), LIFT $H{=}3$ 40.0 (13.3/3.3), both same seed.}
\label{tab:v2grid}
\begin{center}
\begin{tabular}{lccc}
\toprule
...
\midrule
LLaDA-8B & v2 champion & 41.7 / 39.6 & 39.8 / 42.1 & 40.0 / 39.0 & 40.5$\pm$1.0 / 40.2$\pm$1.6 \\
...
\midrule
Dream-7B & v2 champion & 46.1 / 44.6 & \textbf{47.4} / 44.7 & 43.1 / 45.2 & 45.5$\pm$2.2 / 44.8$\pm$0.3 \\
...
\midrule
LLaDA-1.5 & v2 champion & 40.8 / 40.4 & 40.3$^\dagger$ / 42.2$^\dagger$ & 41.4 / 40.3 & 40.8$\pm$0.6 / 41.0$\pm$1.1 \\
...
\begin{table}[t]
\caption{The refutation campaign (core average, mean $\pm$ sd over seeds 42--44; sel.\ / final). Flagship and reference rows repeated for comparison. All campaign runs single-GPU (layout-equivalence certified in Appendix~\ref{app:layout}). $^\ddagger$single seed; $^{\S}$two of three seeds (remaining evaluations in flight at submission; no pending cell can change a verdict).}
\label{tab:refute}
\begin{center}
\resizebox{\linewidth}{!}{%
\begin{tabular}{lcccc}
...
per-sample cap & $39.0{\pm}0.7$ / $40.6^{\S}$ & $43.6^{\S}$ / $43.8{\pm}0.5$ & $41.0^{\S}$ / $40.2{\pm}0.2$ & $38.8^\ddagger$ / --- \\
two-sided & $40.1{\pm}0.7$ / $39.9{\pm}1.0$ & $\mathbf{44.9{\pm}0.5}$ / $44.6^{\S}$ & $41.1^\ddagger$ / $41.0{\pm}0.9$ & $42.5^\ddagger$ / $39.8{\pm}1.7$ \\
combined & --- / $39.6{\pm}1.1$ & $43.2^{\S}$ / $42.3{\pm}0.2$ & --- & --- \\
step-value scaling & $39.9^\ddagger$ / --- & --- & --- & --- \\
\midrule
...
Result (Figure~\ref{fig:aa}): across the completed pairs the gaps are $+0.6, +1.6, +0.2, -0.4, -1.9, -0.3, +0.1, +0.5$ core points (positive = 4-GPU higher)---both signs, mean $+0.05$, every gap inside the configuration's own cross-seed spread---and on the LLaDA-1.5 selected column the three-seed means coincide exactly ($40.8$ versus $40.8$). We conclude the two layouts draw from the same result distribution, differing like a seed re-draw; no comparison in this paper is confounded by hardware layout.

\begin{figure}[h]
\centering
\includegraphics[width=0.5\textwidth]{figs/fig_aa.pdf}
\caption{\textbf{Layout equivalence.} Paired same-configuration, same-seed runs trained on 1 GPU (accumulation 8) versus 4 GPUs (accumulation 2). Points scatter around the identity line within the shaded typical seed-noise band ($\pm 0.8$); the mean paired gap is $+0.05$ core points.}
\label{fig:aa}
\end{figure}
