...
The design rests on what a gradient is for. The gradient of a masked token teaches one thing: produce this token from this context. We want every such lesson to encode an inference the model can reuse, never an answer it must memorize---and which of the two a token delivers depends entirely on what is visible when it is supervised. A token whose determining evidence is hidden is unanswerable; supervising it rewards producing the answer without justification, a lesson that cannot transfer. A token that is already obvious contributes no gradient at all. The lessons worth paying for lie between the two: tokens whose answer follows from the visible evidence through an inference the model has not yet mastered. Our learnability measurement detects exactly this regime---a token whose confidence rises when evidence appears carries an unfinished inference worth completing. So we reveal context not to assist the model but to design its exercises: the premises go on the table, and every supervised token demands a derivation the model is capable of learning, at the greatest difficulty it can learn from.
