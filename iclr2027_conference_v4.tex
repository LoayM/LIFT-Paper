...
\subsection{Overview}
The training process of diffusion language models can be viewed as follows: a training step resembles a lesson. The visible tokens provide the material, the masked tokens are the exercises, and the loss weights determine how those exercises are graded. Under this view, designing the corruption process amounts to designing a good lesson. The key difference from human teaching is that, for language models, the quantities we care about are measurable. In order to better understand the behavior of training dLLMs, we estimate before training which masked tokens are most likely to benefit from additional context, and which visible tokens provide that benefit. Rather than relying on heuristics, we begin by measuring these two quantities (Section~\ref{sec:measurements}).

These measurements guide every component of the method. First, we construct the context by selecting visible tokens that are predicted to provide the largest learning benefit for the chosen masked tokens. This naturally leads to a submodular objective that balances providing useful context against keeping enough challenging prediction targets (Section~\ref{sec:objective}). Second, we assign loss weights according to the observed benefit each token actually receives, allocating the gradient budget through proportional fairness (Section~\ref{sec:weighting}). Finally, instead of fixing the reveal budget, we adapt it during training by maintaining the masked tokens near the measured learnability region. This is implemented with a simple Lagrangian update that adjusts the reveal budget automatically (Section~\ref{sec:lagrangian}). An overview of the complete pipeline is shown in Figure~\ref{fig:overview}.

Overall, the method follows a single principle: whenever possible, decisions are driven by measurements rather than fixed heuristics. The measured curves determine the target behavior, while adaptive variables enforce the corresponding constraints throughout training.

\subsection{Two measurements}
\label{sec:measurements}
\paragraph{The learnability curve.}
For 100 held-out training examples, each evaluated under four independent noise realizations, we measure how additional context affects prediction quality. We corrupt each example at two nested noise levels, $t+\rho$ and $t+\rho/2$, using shared uniform random variables so that the sparser mask is always a subset of the denser one. For each of the 957,563 tokens masked in both corruptions, we compute the increase in gold log-probability after revealing the additional context (the \emph{context gain}). Multiplying this by the residual-difficulty valuation $(1-p)$ yields a learnability sample, which we average within (confidence bin $\times$ noise bin) cells to estimate $\lambda(p,t)$.

We also report the raw context-gain curve without the valuation factor. It peaks around $p\approx0.5$--$0.8$, showing that additional context is most useful for tokens of intermediate confidence. After applying the valuation factor, the learnability curve (Figure~\ref{fig:curves}, left) peaks consistently around $p\approx0.22$ across all five noise bins, while only its magnitude changes with the corruption level. Repeating the measurement on Dream-7B yields the same qualitative behavior, with the peak shifting only slightly to $p\approx0.26$ (Figure~\ref{fig:invariance}).

A three-parameter model, $\lambda(p)\propto p^a(1-p)^b$, fits all noise bins well ($R^2=0.93$--$0.99$). The curve is also stable across data splits, reproducing the measured cell means with correlation $r=0.967$ and shifting the peak by only $0.01$. In contrast, common weighting heuristics such as $1-p$ and $p(1-p)$ disagree with the measurements, placing too much weight on low-confidence tokens or peaking at $p=0.5$, respectively.

\paragraph{The support curve.}
Our second measurement asks whether the identity of the revealed tokens matters when the reveal budget is fixed. For each combination of supervised targets, noise level, and reveal budget, we evaluate eight random reveal sets (25,217 targets in total). For every target, we record its attention mass on the revealed tokens (its \emph{support} $S_i$) and the resulting gold log-probability.

Since only the reveal set changes within each configuration, regressing prediction quality on support with per-target fixed effects estimates the dose-response function $\phi$, assuming that attention support is the main channel through which reveal-set identity affects prediction quality. The estimated response is monotonic in every noise bin and exhibits mild saturation, with half-saturation between support values of $0.05$ and $0.2$. Its explanatory power increases by roughly $30\times$ from low to high noise, indicating that reveal-set identity becomes increasingly important as masking grows heavier.

A saturating model consistently fits the measurements better than a linear model, improving relative $R^2$ by about $20\%$ in the highest-noise bins. In contrast, a hard-saturation (coverage-style) model performs worst, matching our empirical observation that coverage-based reveal strategies consistently underperform attention-based selection.
