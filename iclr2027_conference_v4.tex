...
Choosing reveals well requires a score: how much does the model benefit from supervising token $i$ when the set $R$ is revealed? We factor this benefit into two quantities with distinct roles. The first, the \emph{teaching value} $\lambda(p_i)$, asks whether token $i$ is worth supervising at all, given the model's current confidence $p_i$ in it. The second, the \emph{support} $\phi(S_i(R))$, asks what fraction of that value the particular reveal set $R$ delivers to it. The benefit is their product, and both quantities are instantiated from measurements performed once on the base model (Appendix~\ref{sec:measurements}), not from tuning. The factorization encodes our central hypothesis: a token benefits from supervision precisely when it responds to evidence while headroom remains---responsiveness being the observable signature of an inference the model has begun but not completed, which is the regime where a gradient strengthens reasoning rather than memorization. The measurements locate that regime; the training experiments of Section~\ref{sec:ablations} test, and repeatedly fail to refute, the hypothesis.
...
\paragraph{The selection objective.} With the two quantities in place, the function writes itself: reveal the set $R$ (of size set by the budget of Section~\ref{sec:lagrangian}) that maximizes the predicted benefit delivered to the tokens that remain supervised,
\begin{equation}
F(R) \;=\; \sum_{i \notin R} \lambda(p_i)\,\phi\big(S_i(R)\big).
\label{eq:objective}
\end{equation}
The sum over $i \notin R$ builds in the price of every reveal: a token moved into the context is a token no longer taught, so each candidate reveal must earn more through the tokens it supports than it forfeits by leaving the lesson. The optimizer consequently constructs context from tokens that are cheap to give away---already mastered, or currently hopeless---and protects the teachable middle. $F$ is submodular for any saturating $\phi$ (Proposition~\ref{prop:submodular}), so greedy construction of $R$ is provably near-optimal \citep{buchbinder2014submodular}; it costs $\sim$140ms per step, and prior attention heuristics emerge as its limiting cases (Proposition~\ref{prop:limits}). 
