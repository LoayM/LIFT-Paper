...
\subsection{Overview: from a hypothesis to a training step}

Our hypothesis is pedagogical, and we commit to it literally: \emph{a training step is a lesson}. The visible tokens are the material, the masked tokens are the exercises, and the loss weights are the grading scheme. If that is true, then designing the corruption well requires exactly what designing a lesson requires --- knowing which exercises the student is ready to learn from, and which material helps with which exercise. The crucial observation is that for a language model, unlike for a classroom, both of these are \emph{measurable}: the model itself can be probed, cheaply and before training, for how much a token benefits from context and for which context carries that benefit. So where prior work chose a proxy and committed to it, we begin by measuring the two quantities the hypothesis says a lesson needs (Section~\ref{sec:measurements}).

Measurement alone, however, does not make a method; the curves must be turned into decisions, and each decision the step faces inherits its answer from the hypothesis. \emph{Which context to construct?} Reveal the tokens that deliver the most predicted learning to the exercises worth doing --- an objective whose structure (submodular, and self-paying: every hint costs the lesson it forfeits) resolves the inherent conflict between giving help and keeping exercises (Section~\ref{sec:objective}). \emph{How to grade?} Predictions can be wrong, so the loss weights are set by what the help \emph{actually} accomplished, and because a batch has a fixed amount of gradient to give, the grades are allocated by the unique proportionally fair division (Section~\ref{sec:weighting}). \emph{How much help?} This is the question every predecessor answers with a constant, and it is where the hypothesis pays its way: the right amount of help is whatever keeps the exercises at the measured zone of learnability --- so we impose that as a constraint and let its Lagrange multiplier, a two-line thermostat with a unique equilibrium, discover the budget during training (Section~\ref{sec:lagrangian}). Figure~\ref{fig:overview} assembles the step.

The discipline that emerges --- and the sentence this paper defends --- is that \emph{every fixed quantity is a measured curve, and every adaptive quantity is a multiplier enforcing a constraint on a measured signal}. Nothing else is part of the method: alternative budgets, weightings, and corruption designs appear in this paper only as ablations and attempted refutations (Sections~\ref{sec:v2results}, \ref{sec:ablations}, and \ref{sec:refute}).

\subsection{Two measurements}
\label{sec:measurements}

The lesson hypothesis poses two empirical questions: which exercises can this student learn from right now, and which material teaches them? Neither needs to be guessed. Both admit direct, randomized measurement on the base model, at a cost of about twenty-five GPU-minutes each.
